...
app.post('/api/order', async (req, res) => {
    ...
    let msg = `🛒 *YANGI BUYURTMA*\n🆔 ID: ${orderId}\n👤 Mijoz: ${username}\n📦 Mahsulot: ${rate}\n🔢 Chek: ${receiptCode || "Rasm yuborildi"}`;
...
// Admin uchun bot komandalari
bot.on('message', (msg) => {
    if (msg.text && msg.text.startsWith('/send_')) {
        const [cmd, id, ...acc] = msg.text.split(' ');
        const orderId = cmd.replace('/send_', '');
        let orders = readOrders();
        let order = orders.find(o => o.id === orderId);
        if (order) {
            order.status = 'Yetkazildi';
            order.account = acc.join(' ');
            writeOrders(orders);
            bot.sendMessage(CHAT_ID, "✅ Saqlandi!");
        }
    }
});
